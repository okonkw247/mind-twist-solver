...
const PlayCube = () => {
  ...
  const [energy] = useState({ current: 10, max: 10 });
  const [progress] = useState({ current: 12, total: 45 });
                ...
                <div className="mt-2 text-center">
                  ...
                  <div className="flex gap-0.5 justify-center mt-1">
                    {[1, 2, 3].map(star => (
                      <Star
                        key={star}
                        className={`w-3 h-3 ${star <= level.stars ? 'text-gold fill-gold' : 'text-muted-foreground'}`}
                      />
                    ))}
                  </div>
                </div>
            ...
            >
              ...
              <div className="flex justify-center items-center gap-4 mb-8">
                <div className="flex flex-col items-center">
                  ...
                  <button onClick={() => setSelectedMinutes(prev => Math.min(prev + 1, 59))} className="p-2 rounded-lg hover:bg-secondary">
                    <ChevronUp className="w-5 h-5" />
                  </button>
                  <span className="text-4xl font-bold w-16 text-center tabular-nums">{selectedMinutes}</span>
                  <button onClick={() => setSelectedMinutes(prev => Math.max(prev - 1, 0))} className="p-2 rounded-lg hover:bg-secondary">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <button onClick={() => setSelectedSeconds(prev => prev >= 55 ? 0 : prev + 5)} className="p-2 rounded-lg hover:bg-secondary">
                    <ChevronUp className="w-5 h-5" />
                  </button>
                  <span className="text-4xl font-bold w-16 text-center tabular-nums">{selectedSeconds}</span>
                  <button onClick={() => setSelectedSeconds(prev => prev <= 0 ? 55 : prev - 5)} className="p-2 rounded-lg hover:bg-secondary">
                    <ChevronDown className="w-5 h-5" />
                  </button>
                </div>
              </div>
              <button onClick={() => setShowTimeModal(false)} className="btn-primary w-full py-4 text-lg font-bold">
                OK
              </button>
            </motion.div>
          </motion.div>
